...
extern float g_base_speed;    /* 基础速度 (驱动板单位) */
extern float g_base_rpm;      /* 兼容旧命令, 实际映射到 g_base_speed */
extern float g_turn_p;
extern float g_turn_d;
extern float g_pivot_speed;    /* 原地转弯速度 (驱动板单位, 默认200) */
extern float g_corner_fwd_ms;  /* 直角弯前冲时间 (ms, 默认500, 命令f调节) */
...
extern float   g_dbg_centroid;    /* 当前重心 -3.5~+3.5 */
extern uint8_t g_dbg_active_cnt;  /* 激活传感器数量 0-8 */
extern uint8_t g_dbg_active_bits; /* 激活传感器位图 (bit0=CH0 ... bit7=CH7) */
extern uint8_t g_dbg_track_state; /* 状态机 0=NORMAL 1=CORNER_FWD 2=CORNER_TURN 3=LOCKED */
extern int16_t g_dbg_corner_dir;  /* 转弯方向 +1=右 -1=左 */
extern float   g_dbg_turn;        /* PD输出 */
